...
let skills = [
  { text: '._Android SDK',   position: new THREE.Vector3(-0.2, 0.7, 1.2), color: 0xff5722 },
  { text: '._Kotlin | XML',  position: new THREE.Vector3(-0.2, 0.6, 1.2), color: 0x3f51b5 },
  { text: '._Compose',       position: new THREE.Vector3(-0.2, 0.5, 1.2), color: 0x4caf50 },
  { text: '._Retrofit',      position: new THREE.Vector3(-0.2, 0.4, 1.2), color: 0x009688 },
  { text: '._Python',        position: new THREE.Vector3(-0.2, 0.3, 1.2), color: 0xffeb3b },
  { text: '._SQL | MongoDB', position: new THREE.Vector3(-0.2, 0.2, 1.2), color: 0x9c27b0 },
];
...
// --- NEW SKILLS CAMERA POSITIONS ---
let skillsCameraPos = {
  x: 0.3,   // Center/right of room
  y: 0.45,  // Eye level
  z: 0.5,   // Near the PC
};
...
let skillsCameraRot = {
  x: 0,
  y: Math.PI / 2, // Turn left to face the left wall
  z: 0,
};
...
gltfLoader.load(
  ...
  function (room) {
    ...
    // --- NEW: Add the skills menu listener ---
    skillMenuListener();
...
function loadIntroText() {
  ...
  loader.load('fonts/unione.json', function (font) {
    ...
    const titleGeo = new TextGeometry('KESHAV   RAJ', {
      font: font,
      size: 0.08,
      height: 0.01,
    });
  ...
  loader.load('fonts/helvatica.json', function (font) {
    ...
    const subTitleGeo = new TextGeometry(
      'Android Developer / App Designer / Compose & XML \n\n +91 7292870631',
      {
        font: font,
        size: 0.018,
        height: 0,
...
// --- NEW: Function to reset skills visibility ---
function resetSkills() {
  if (skills.length === 0) return;

  skills.forEach((skill, i) => {
    gsap.to(skill.mesh.material, {
      opacity: 0,
      duration: 1,
    });
    gsap.to(skill.mesh.scale, {
      x: 0,
      y: 0,
      z: 0,
      duration: 0,
...
// --- NEW: Skill menu listener function ---
function skillMenuListener() {
  const loader = new FontLoader();
  loader.load('fonts/helvatica.json', function (font) {
    skills.forEach((skill, i) => {
      const textGeometry = new TextGeometry(skill.text, {
        font: font,
        size: 0.05,
        color: skill.color,
        height: 0.004,
      ...
      const skillText = new THREE.Mesh(textGeometry, textMaterial);
      skillText.position.copy(skill.position);
      skillText.rotation.y = Math.PI / 2; // Rotate to face the camera from the left wall
      skillText.scale.set(0, 0, 0); // Start with scale 0
      skills[i].mesh = skillText;
      scene.add(skillText);
    });
  });
  ...
  document
    ...
    .addEventListener('click', function (e) {
      ...
      disableOrbitControls();
      resetBookCover();
      resetProjects(); // Hide projects when showing skills
      gsap.to(camera.position, {
        ...skillsCameraPos,
        duration: 1.5,
      });
...
function init3DWorldClickListeners() {
  ...
  window.addEventListener('click', function (e) {
    ...
    // --- NEW: Add skill-menu to the click prevention list ---
    const skillsBtn = document.getElementById('skill-menu');
...
function initResponsive(roomScene) {
  if (isMobile) {
    ...
    // --- NEW: Skills responsive positioning ---
    skillsCameraPos = {
      x: -1.1,
      y: 0.82,
      z: 0.5,
